...
def register_lddmm(fixed_img, moving_img, moving_segs=None):
    """
    ...
    if moving_segs is not None:
        warped_segs = []
        for m_seg in moving_segs:
            warped_seg = ants.apply_transforms(
                fixed=fixed_img, moving=m_seg, transformlist=transform['fwdtransforms'], interpolator='nearestNeighbor'
            )
            warped_segs.append(warped_seg)

    return warped_img, warped_segs, transform['fwdtransforms']
...
def optimize(dataset, image_paths, seg_paths, save_dir, template_path):
    ...
    for _, _, _, _, _, path in tqdm(save_loader):
        ...
        for seg_path in seg_paths:
            if os.path.exists(f'{seg_path}/{seg_name}'):
                moving_seg = ants.image_read(f'{seg_path}/{seg_name}')
                moving_segs.append(moving_seg)

        # if os.path.exists(pet_path):
        #     moving_segs.append(ants.image_read(pet_path))
        
        if len(moving_segs) == 0:
            moving_segs = None
        
        # LDDMM (SyN)
        # print(pet_path)
        # exit()
        warped_ants, warped_segs, transform = register_lddmm(fixed_ants, moving_ants, moving_segs)

        warped_ants.to_filename(f"{save_dir}/{save_name}_warped.nii.gz")
        if warped_segs is not None:
            for warped_seg, seg_path in zip(warped_segs, seg_paths):
                warped_seg.to_filename(f"{save_dir}/{save_name}_seg.nii.gz")
            # warped_segs[-1].to_filename(f'{save_dir}/{save_name}_warped_PET.nii.gz')
...
if __name__ == '__main__':
    ...
    seg_dir = ["data/DLBS_label_core"]
    save_dir = "data/DLBS_SyN"
    template_path = "data/mni152_resample.nii"
    optimize(dataset, root_dir, seg_dir, save_dir, template_path)

    # dataset = 'FDG_MRI'
    # pet_dir = "data/FDG_PET_percent"
    # seg_dir = ["data/FDG_label", "data/FDG_label_cortex"]
    # save_dir = "data/FDG_MRI_SyN"
    # template_path = "data/mni152_resample.nii"
    # optimize(dataset, pet_dir, seg_dir, save_dir, template_path)
